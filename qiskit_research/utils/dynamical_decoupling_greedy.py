...
class PadDynamicalDecoupling(BasePadding):
    ...
    def __init__(
        ...
        self._dd_sequence_lengths = [dict()] * len(self._dd_sequences)
        self._sequence_phase = []

    def _pre_runhook(self, dag: DAGCircuit):
        super()._pre_runhook(dag)

        num_pulses = [len(dd_sequence) for dd_sequence in self.dd_sequences]
        ...
        # Check if each DD sequence is identity
        for n_pulses, dd_sequence in zip(num_pulses, self._dd_sequences):
            if n_pulses != 1:
                if n_pulses % 2 != 0:
                    raise TranspilerError("DD sequence must contain an even number of gates (or 1).")
                noop = np.eye(2)
                for gate in self._dd_sequences:
                ...
                self._sequence_phase.append(np.angle(noop[0][0]))
            else:
                self._sequence_phase.append(0)
        ...
        for qubit in dag.qubits:
            ...
            for i in range(len(self._dd_sequences)):
                sequence_length = []
                dd_sequence = self._dd_sequences[i]
    ...
    def _pad(
        ...
        slack = -1
        for i in range(len(self._dd_sequences)):
            slack = time_interval - np.sum(self._dd_sequence_lengths[i][qubit])
            sequence_gphase = self._sequence_phase[i]
